...
[（3）](./chapter3/3.3.cpp)设从键盘输入一整数的序列：$a_{1}, a_{2}, a_{3}, …, a_{n}$，试编写算法实现：用栈结构存储输入的整数，当 $a_{i} ≠ -1$ 时，将 $a_{i}$ 进栈；当 $a_{i} = -1$ 时，输出栈顶整数并出栈。算法应对异常情况（入栈满等）给出相应的信息。
...
&emsp;&emsp;$A. \ IOIOO \qquad B. \ IOIOIO \qquad C. \ IOIO \qquad D. \ IOIO$

&emsp;② 通过对 ① 的分析，写出一个算法，判定所给的操作序列是否合法。若合法，返回 `true`，否则返回 `false` （假定被判定的操作序列已存入一维数组中）。
...
&emsp;② 写出“从队尾删除”和“从队头插入”的算法。

<br/>

[（9）](./chapter3/3.9.cpp)已知 `Ackerman` 函数定义如下：

&emsp;① 写出计算 `Ack(m, n)` 的递归算法，~~并根据此算法给出 Ack(2, 1) 的计算过程~~。

&emsp;② 写出 `Ack(m, n)` 的非递归算法
...
&emsp;① 写一个算法判断 ~~a~~ `arr` 中所有元素是否互不相同？输出相关信息(`yes/no`)；
...
&emsp;① 增加一个新顶点 ~~v~~ `vex`，~~insVex(G, v)~~ `InsVex` ;

&emsp;② 删除顶点 ~~v~~ `Vex` 及其相关的边，~~DeleteVex(G, v)~~ `DelVex`;

&emsp;③ 增加一条边 ~~<v，w>~~ `<begin_vex, end_vex>`，~~insArc(G, v, w)~~ `InsEdge`;

&emsp;④ 删除一条边 ~~<v，w>~~ `<begin_vex, end_vex>`，~~DeleteArc~~ `DelEdge`。
...
&emsp;~~① 给出适用于计数排序的顺序表定义；~~

&emsp;② 编写实现计数排序的算法；

&emsp;~~③ 对于有 n 个记录的表，关键字比较次数是多少？~~

&emsp;~~④ 与简单选择排序相比较，这种方法是否更好？为什么？~~